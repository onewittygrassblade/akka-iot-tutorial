...
object Device {
  ...
  def apply(groupId: String, deviceId: String): Behavior[Command] =
    Behaviors.setup { context =>
      ...
      new Device(groupId, deviceId).processMessages(None)
    }
}

class Device private (groupId: String, deviceId: String) {
  import Device._
      ...
      .receive[Command] { (context, message) =>
        message match {
          case RecordTemperature(requestId, value, replyTo) =>
            ...
            context.log.info(
              "Device actor {}-{} recorded temperature reading {} with request {}",
              groupId,
              deviceId,
              value,
              requestId
            )
            processMessages(Some(value))
