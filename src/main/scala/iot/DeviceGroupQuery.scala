...
class DeviceGroupQuery(
  ...
  // We have to convert the RespondTemperature replies from the device actor to the message protocol that the
  // DeviceGroupQuery actor understands, i.e. DeviceGroupQueryMessage. For this we use a messageAdapter that wraps
  // the RespondTemperature in a WrappedRespondTemperature, which extends DeviceGroupQueryMessage.
  private val respondTemperatureAdapter = context.messageAdapter(WrappedRespondTemperature.apply)

  // State: already received replies (map) + actors we are still waiting on (set)
